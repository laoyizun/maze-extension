...
//================== 拓展弹射物 ==================
    ...
    export function setPrismLW(b:wave, l: number, w: number, t: number=1000){
        b.prismLength = Math.min(l, scene.screenWidth())
        b.prismWidth = Math.min(w, scene.screenWidth())
        b.lifespan = t
    }
    ...
    //% weight=75
    export function bulletInterval(t: number, p: Sprite, func: () => void) {
        let clock: number
        clock = setInterval(()=>{
            if(Helper.isDestroyed(p)){
                clearInterval(clock)
    ...
    export function getCurEnemyRoom(){
        return curEnemyRoom
    }
